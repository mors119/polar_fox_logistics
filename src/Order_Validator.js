function validateCsv(parsedCsv) {
  ...
  const missingHeaders = REQUIRED_ORDER_HEADERS.filter(
  ...
  const unknownHeaders = headers.filter(
    header => header && !ORDER_CSV_HEADERS.includes(header)
  );
...
function validateOrderRows(rows) {
  ...
  rows.forEach(row => {
    ...
    if (!orderNumber) {
      errors.push(buildOrderRowError_(
        rowNumber, orderNumber, orderItemNumber, "REQUIRED_VALUE", "주문번호는 필수입니다."
      ));
    }

    if (!orderItemNumber) {
      errors.push(buildOrderRowError_(
        rowNumber, orderNumber, orderItemNumber, "REQUIRED_VALUE", "품목별 주문번호는 필수입니다."
      ));
    } else if (seenOrderItemNumbers.has(orderItemNumber)) {
      errors.push(buildOrderRowError_(
        rowNumber, orderNumber, orderItemNumber, "DUPLICATE_IN_FILE", "CSV 내부 품목별 주문번호가 중복되었습니다."
      ));
    } else {
    ...
    if (!productItemCode) {
      errors.push(buildOrderRowError_(
        rowNumber, orderNumber, orderItemNumber, "REQUIRED_VALUE", "상품품목코드는 필수입니다."
      ));
    }

    if (!orderItemName) {
      errors.push(buildOrderRowError_(
        rowNumber, orderNumber, orderItemNumber, "REQUIRED_VALUE", "주문상품명은 필수입니다."
      ));
    }
    ...
    if (!quantityText) {
      errors.push(buildOrderRowError_(
        rowNumber, orderNumber, orderItemNumber, "REQUIRED_VALUE", "수량은 필수입니다."
      ));
    } else if (quantityNumber === null || quantityNumber < 1) {
      errors.push(buildOrderRowError_(
        rowNumber, orderNumber, orderItemNumber, "INVALID_QUANTITY", `수량은 1 이상의 정수여야 합니다: ${quantityText}`
      ));
    }
    ...
    if (!recipient) {
      errors.push(buildOrderRowError_(
        rowNumber, orderNumber, orderItemNumber, "REQUIRED_VALUE", "수령인은 필수입니다."
      ));
    }

    if (!recipientAddress) {
      errors.push(buildOrderRowError_(
        rowNumber, orderNumber, orderItemNumber, "REQUIRED_VALUE", "수령인 주소는 필수입니다."
      ));
    }
    ...
    if (orderNumber) {
      ...
      if (!orderSnapshots[orderNumber]) {
        ...
        const inconsistentField = ORDER_HEADER_CONSISTENCY_FIELDS.find(field =>
          orderSnapshots[orderNumber][field] !== snapshot[field]
        );
