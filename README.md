...
카페24 `상품코드 / 상품명 / 품목코드 / 재고수량` 형식의 `재고수량`은 입고 증가분이 아닌 현재 재고 스냅샷으로 처리합니다. 따라서 동일 파일을 다시 올려도 기존 재고에 중복 가산하지 않습니다.

카페24 현재 재고는 품절 초과 주문으로 `-1`, `-2`처럼 음수가 될 수 있어 스냅샷 형식에서만 음수를 허용합니다. 수동 입고·재입고 양식의 음수 수량은 계속 오류로 처리합니다.
